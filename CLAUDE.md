...
- Docs: `ARCHITECTURE.md` (deep design, incl. the terminal & headless-test
  ...
  `ASSETS.md` (declarative assets: the per-adventure manifest `assets/assets.json`
...
- **`assets/assets.json` is the adventure's asset manifest** (see `ASSETS.md`):
...
- **Placeholder art & voice have a pipeline.** Each adventure lists the real
  assets it still needs in `assets/assets.json`; artists upload raw files into
