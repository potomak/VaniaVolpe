...
framework ticks, and `AnimationData.ms_per_frame` already supports the
slower rate a boil wants (~170 ms/frame ≈ 6 FPS, vs. the 12 FPS default —
boils read as texture, not motion, at low rates). Two small pieces are
new:
...
are affordances of the scene edges, not things). Until real boil art
lands, a placeholder can be generated: a small `tools/` script that
jitters an existing PNG (per-frame random ~1 px displacement noise) into
a 2–3 frame sheet would make the mechanism visible immediately and be
replaced frame-for-frame by hand art later.
...
1. **Part 3** first — smallest engine delta, changes the feel of every
   scene, and the placeholder-boil tool makes it visible without waiting
   for art.
