...
typedef struct chunk_data {
  ...
  // The WAV is optional: a not-yet-recorded dialogue line has its text but no
  // audio, so a missing file loads as a text-only line (chunk == NULL) instead
  // of failing. Plain SFX/music leave this false and a missing WAV is still a
  // hard error.
  bool optional_audio;
